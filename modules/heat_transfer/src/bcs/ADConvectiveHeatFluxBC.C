...
ADConvectiveHeatFluxBC::validParams()
{
  ...
  // Using functors
  params.addParam<MooseFunctorName>("T_infinity_functor",
...
ADConvectiveHeatFluxBC::ADConvectiveHeatFluxBC(const InputParameters & parameters)
  : ADIntegratedBC(parameters),
    ...
    _htc(isParamValid("heat_transfer_coefficient") ? &getADMaterialProperty<Real>("heat_transfer_coefficient") : nullptr),
    _T_infinity_functor(isParamValid("T_infinity_functor") ? &getFunctor<ADReal>("T_infinity_functor") : nullptr),
    _htc_functor(isParamValid("heat_transfer_coefficient_functor") ? &getFunctor<ADReal>("heat_transfer_coefficient_functor") : nullptr)
{
  if (_T_infinity || _htc)
  {
    if (_T_infinity_functor || _htc_functor)
      paramError("T_infinity_functor", "Either material properties or functors should be specified");
    if (!_htc)
      paramError("heat_transfer_coefficient", "Heat transfer coefficient material property must be specified");
    if (!_T_infinity)
  ...
  {
    if (!_htc_functor)
      paramError("heat_transfer_coefficient_functor", "Heat transfer coefficient functor must be specified");
    if (!_T_infinity_functor)
      paramError("T_infinity_functor", "Far field temperature functor must be specified");
  }
  else
    paramError("T_infinity", "Far field temperature and heat transfer coefficients must be specified");
}
...
ADConvectiveHeatFluxBC::computeQpResidual()
{
  ...
  else
  {
    ...
    return -_test[_i][_qp] * (*_htc_functor)(space_arg, time_arg) * ((*_T_infinity_functor)(space_arg, time_arg) - _u[_qp]);
  }
}
